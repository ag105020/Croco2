...
def gft(FileName):   #This function generate files from a certain folder 
    return genfromtxt('C:\\Users\\Keiin\\Google Drive\\0 Paper\\68 Masuda 2013\\Data\\'+FileName+'.csv',delimiter=',')
...
def ModelRun(NfixFactor):
    ...
    Ngrowth = ((1+E)*Dd)/Apho     #(molN molC-1) Growth dependent N
    print(((1+E))/Apho)
    ...
    Cn = (Nin-DONideal)/(Qn0-Nfix/Dd)   #(molC m-3) C in N limited case 
    Cn[Cn<0] = nan   #(molC m-3) negative values are removed
    Cp = Pin/Qp0     #(molC m-3) C in p limited case
    ...
    #---------------------------------------------------
    Qn = (Nin+C*Nfix/Dd-DONideal)/C     #(molN molC-1) Nitrogen quota (Kei213-20)
    #Qn = Ngrowth + Nother + Nstore
    Qp = Prna + Pother + Pstore      #(molP molC-1) P quota

    Nexc = Dd*DONideal/C     #(molN molC-1 d-1) N excretion rate per carbon
    P = Pin - Qp*C           #(molP m-3) PO4 concentration in the culture
  ...
  #  legend(loc=4)
    ...
    if NfixFactor==1:
        sup(10)
        stackplot(Dd,Nother*ones(size(Dd)),Ngrowth,Nstore,colors=('b','cyan','r'))
        ylabel('N:C' + RatioUnit)
        xy0()
        #--------Manual Legend----------
        ...
        sup(11)
        stackplot(Dd,Pother*ones(size(Dd)),Prna,colors=('b','cyan'))
        ylabel('P:C' + RatioUnit)
        xy0()
            #--------Manual Legend----------
        Names = ['$\mathit{P_{Other}}$','$\mathit{P_{Growth}}$']
        Colors = ['blue','cyan']
        for i in arange(size(Names)):
            plot([],[],linewidth=12,color=Colors[-i-1],label=Names[-i-1])
        legend(loc=2)
